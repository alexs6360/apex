/* Storm history lookup.
   ...
   Ported from riseroofingms.com's storm history tool. Data is static:
   data/storm-index.json plus data/hail-YYYY.json and data/reports-YYYY.json
   are generated at build time by tools/build-storm-data.mjs and committed.
   No backend, no API in the request path except Mapbox for the basemap and
   for turning the typed address into a coordinate.

   Versions targeted, checked against docs.mapbox.com:
     Mapbox GL JS  v3.30.0
...
(function () {
  ...
  /* Substituted by tools/substitute-mapbox-token.sh from the MAPBOX_TOKEN
     environment variable, read from a local, gitignored .env.local — see
     README.md. The token reaches the browser either way — a public pk. token
     in a static page is normal — but keeping it out of the committed source
     means rotating it does not need a find-and-replace across the repo.
     Restrict it by URL (apexexteriorsmidsouth.com) in the Mapbox account. */
  ...
  /* Rise's build.sh scraped this file for every StormGrid member it calls and
     injected the list at build time. Apex has no build step, so it is
     hand-maintained here instead — the three actually called below:
     bucketOf, fitEnvelopes, GRID_DEG. Keep this in sync if either changes. */
  var GRID_API = "bucketOf,fitEnvelopes,GRID_DEG".split(",").filter(Boolean);
  ...
  /* Proximity bias for address search. CENTER is the centroid of the whole
     ...
     road names above the streets people actually type. Memphis is the largest
  ...
  /* Set while prefillFromQuery()'s synthetic submit is in flight. A typed
     ...
     told so; an address arriving from another page's teaser search failing
     to geocode identically here is not something to alarm them with — they
  ...
  function initMap() {
    ...
    /* Loud on purpose. A bad or URL-restricted token still resolves the style
       JSON (that request has no such restriction) and then fails silently on
       every tile after — an empty canvas with a working attribution bar,
       which looks like a rendering bug and is actually an auth rejection. */
    map.on("error", function (e) {
      var msg = (e && e.error && e.error.message) || "unknown map error";
      console.error("storm-history: map error —", msg, e && e.error);
      if (mapNote) {
        mapNote.textContent = "Map failed to load (" + msg + "). The lookup below still works.";
    ...
    /* The map is constructed as soon as loadData() resolves, which can race
       layout in a hidden tab or a not-yet-painted flex/grid ancestor — a
       Mapbox canvas built at 0x0 stays blank even after the container
       gets real size, because GL never re-reads it on its own. Watching the
       container directly catches that regardless of what caused it, rather
  ...
  /* Contours only ever represent one storm day, so a date is always selected
     ...
     had. The default is the most recent event; events[0] after the newest-
     first sort in eventsAt(). */
  ...
  function recordSearch(address, summary, topDate) {
    ...
    var body = new URLSearchParams({
      "form-name": "apex-storm-lookup",
      address: address,
      summary: "Searched " + new Date().toISOString().slice(0, 10) + " — " + summary,
      /* The date the notification should draw, if we ever build one. Still no
         coordinate: a date is not a location, and the address string is the
         same one already in the field above. */
      storm_date: topDate || "",
    });
    fetch("/", {
      method: "POST",
  ...
  function runLookup(lon, lat, name, typed) {
    ...
    if (!inServiceArea(lon, lat)) {
      say("That address is outside our coverage area, so we don't have storm history for it. " +
          "We cover the Memphis metro — call us and we'll tell you whether we can still help.");
      resetPanel();
      return;
    }
    ...
    /* Chosen once, used twice: the date the map opens on is the date the
       lead notification draws, so the email and the homeowner's screen show
       the same storm. Defaults to the most recent event — events arrive
       newest-first, so that is simply the first row. */
  ...
  form.addEventListener("submit", function (e) {
    ...
    var wasAutoLookup = autoLookupPending;
      ...
      .then(function (data) {
        ...
        if (!f) {
          /* No longer "in our service area": coverage is a separate answer
             ...
             resolve the text at all. An address handed off from the
             homepage teaser gets the normal empty state instead — the
             reader already searched once, on the page they searched from. */
          if (!wasAutoLookup) {
        ...
        if (wasAutoLookup) scrollToResults();
      })
      .catch(function () {
        if (!wasAutoLookup) {
          say("Something went wrong looking that up. Call us and we'll check it for you.");
  ...
  function scrollToResults() {
    var target = document.querySelector(".sh-workspace");
