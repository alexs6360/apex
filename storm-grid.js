...
const GRID_DEG = 0.05;   // ~5km: three radars see one storm and report
...
function cellsToRings(cells, grid) {
  const half = grid / 2;
...
function smoothRing(ring, iterations = 2, t = 0.22) {
...
/* ---- morphological shaping -------------------------------------------------
   ...
   in the shape itself. */
const SUBGRID = 6;              // sub-cells per cell edge (~0.93km each)
const CLOSE_R = 3;              // half a cell: bridges 1-cell gaps only
const OPEN_R = 2;               // rounds tips and trims spurs
...
function principalAxis(pts, fallback) {
...
const ENV_SAMPLES = 160;   // profile samples along the axis
const DENSITY_BONUS = 0.25; // widest where the cells are densest
...
/* Fit nested envelopes for one cluster.
   ...
   nest. */
function fitProfiles(pts, vals, bands, L, W) {
...
function concaveProfile(us, w, support) {
...
const VALLEY_FRAC = 0.55;
function valleyCut(us, w) {
  let peak = 0;
...
function fitEnvelopes(pts, vals, bands, L, W, linkKm) {
...
/* The concave hull is piecewise linear, so a band resting on two hull vertices
   ...
   straight. */
const ELLIPSE_BLEND = 0.55;
function roundProfile(us, w, support) {
  if (!support) return w;
  const [lo, hi] = support;
...
function simplifyRing(ring, tolM, lat) {
...
window.StormGrid = { CLOSE_R: CLOSE_R, DENSITY_BONUS: DENSITY_BONUS, ELLIPSE_BLEND: ELLIPSE_BLEND, ENV_SAMPLES: ENV_SAMPLES, ENV_SMOOTH: ENV_SMOOTH, GRID_DEG: GRID_DEG, MAX_SPLIT_DEPTH: MAX_SPLIT_DEPTH, OPEN_R: OPEN_R, SIMPLIFY_M: SIMPLIFY_M, SUBGRID: SUBGRID, VALLEY_FRAC: VALLEY_FRAC, bridgeGaps: bridgeGaps, bucketOf: bucketOf, cellKey: cellKey, cellsToRings: cellsToRings, cluster: cluster, concaveProfile: concaveProfile, fitEnvelopes: fitEnvelopes, fitProfiles: fitProfiles, kmFrame: kmFrame, preferred: preferred, preferredReport: preferredReport, principalAxis: principalAxis, profileToRing: profileToRing, ringArea: ringArea, roundProfile: roundProfile, shapeCells: shapeCells, simplifyRing: simplifyRing, smoothRing: smoothRing, valleyCut: valleyCut, };
